...
// class constructor
    function __construct() {
      ...
      if ( defined('MODULE_SHIPPING_FLAT2_STATUS') ) {
      ...
      // disable the module if the order contains a product not valid for uship
      if ($this->enabled == true) {
          $this->enabled = $this->getUShipStatus();
      }	
    }

	
    function getUShipStatus() {
        global $order;
        $whereclause = "";
        $id = 0;
        $primero = true;
        ...
        if ($n > 0) {
            ...
            $uship_check_query = tep_db_query("select sum(products_uship_status) as uship2 from products where " . $whereclause);
