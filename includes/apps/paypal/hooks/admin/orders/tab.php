    ...
    function execute() {
      ...
      if ( tep_db_num_rows($ppstatus_query) ) {
        ...
        if ( isset($status['Transaction ID']) ) {
          ...
          $info_button = $this->_app->drawButton($this->_app->getDef('button_details'), tep_href_link('orders.php', 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oID . '&action=edit&tabaction=getTransactionDetails'), 'primary', null, true);
      ...
      if ( ($status['Pending Reason'] == 'authorization') || ($status['Payment Status'] == 'In-Progress') ) {
        ...
        if ( !tep_db_num_rows($v_query) ) {
          ...
          if ( $capture_total > 0 ) {
            ...
            $capture_link = tep_href_link('orders.php', 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $order['orders_id'] . '&action=edit&tabaction=doCapture');
      ...
      if ( $status['Pending Reason'] == 'authorization' ) {
        ...
        if ( !tep_db_num_rows($v_query) ) {
          ...
          if ( $capture_total > 0 ) {
            ...
            $void_link = tep_href_link('orders.php', 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $order['orders_id'] . '&action=edit&tabaction=doVoid');
...
EOD;
    ...
    function getRefundButton($status, $order) {
      ...
      $ppr_query = tep_db_query("select comments from orders_status_history where orders_id = '" . (int)$HTTP_GET_VARS['oID'] . "' and orders_status_id = '" . (int)OSCOM_APP_PAYPAL_TRANSACTIONS_ORDER_STATUS_ID . "' and comments like 'PayPal App: %' order by date_added desc");
      ...
      if ( $can_refund === true ) {
        ...
        $refund_link = tep_href_link('orders.php', 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $HTTP_GET_VARS['oID'] . '&action=edit&tabaction=refundTransaction');
