...
if (!defined('ORDER_EDITOR_PAYMENT_DROPDOWN')) { // if configs not installed
    ...
    tep_redirect('edit_orders.php' . (isset($_GET['oID']) ? 'oID=' . $_GET['oID'] : ''));
}
...
if (($action == 'edit') && ($order_exists == true)) {
    ...
    <?php } ?>
                            ...
                            <li><?php echo tep_draw_button(IMAGE_BACK, 'arrow-1-w', tep_href_link('orders.php', tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $_GET['oID'] . '&action=edit')); ?></li>
                        </ul>
    ...
    if (tep_db_num_rows($orders_history_query)) {
        while ($orders_history = tep_db_fetch_array($orders_history_query)) {
            ...
            if (ORDER_EDITOR_USE_AJAX == 'true') {
                echo tep_draw_textarea_field("update_comments[" . $orders_history['orders_status_history_id'] . "][comments]", "soft", "40", "5", "" . tep_db_output($orders_history['comments']) . "", "onChange=\"updateCommentsField('update', '" . $orders_history['orders_status_history_id'] . "', 'false', encodeURIComponent(this.value))\"") . '' . "\n" .
                '    </td>' . "\n";
            } else {
                echo tep_draw_textarea_field("update_comments[" . $orders_history['orders_status_history_id'] . "][comments]", "soft", "40", "5", "" . tep_db_output($orders_history['comments']) . "") . '' . "\n" .
                '    </td>' . "\n";
            }
